...
/// </remarks>
[DisplayStringFormat("{negative}/{positive} {modifier}")]
[DisplayName("Positive/Negative Binding with modifier")]
public class AxisCompositeWithModifier : InputBindingComposite<float>
{

    [InputControl(layout = "Button")] public int modifier = 0;
    ...
    [Tooltip("Value to multiply the result by.")]
    ...
    {
        var modifierValue = context.ReadValueAsButton(modifier);
        ...
        if (negativeIsActuated)
            return (mid - (mid - minValue) * negativeValue) * multiplier;

        return (mid + (maxValue - mid) * positiveValue) * multiplier;
    }
    ...
    static AxisCompositeWithModifier()
    {
        ...
        InputSystem.RegisterBindingComposite<AxisCompositeWithModifier>();
    }
